...
pkg_cfg+="--with-hdf5=\"$BLDR_HDF5_BASE_PATH\" "
pkg_cfg+="--with-hdf5-incdir=\"$BLDR_HDF5_INCLUDE_PATH\" "
pkg_cfg+="--with-hdf5-libdir=\"$BLDR_HDF5_LIB_PATH\" "

pkg_cfg+="--with-phdf5=\"$BLDR_PHDF5_BASE_PATH\" "
pkg_cfg+="--with-phdf5-incdir=\"$BLDR_PHDF5_INCLUDE_PATH\" "
pkg_cfg+="--with-phdf5-libdir=\"$BLDR_PHDF5_LIB_PATH\" "

pkg_cfg+="--with-netcdf=\"$BLDR_NETCDF_BASE_PATH\" "
pkg_cfg+="--with-netcdf-incdir=\"$BLDR_NETCDF_INCLUDE_PATH\" "
pkg_cfg+="--with-netcdf-libdir=\"$BLDR_NETCDF_LIB_PATH\" "

pkg_cfg+="--with-nc4par=\"$BLDR_PNETCDF_BASE_PATH\" "
pkg_cfg+="--with-nc4par-incdir=\"$BLDR_PNETCDF_INCLUDE_PATH\" "
pkg_cfg+="--with-nc4par-libdir=\"$BLDR_PNETCDF_LIB_PATH\" "

if [[ $BLDR_SYSTEM_IS_LINUX == true ]]
then
    if [[ -f "/usr/lib64/libibverbs.so" ]]
    then
        pkg_cfg+="--with-infiniband=/usr/lib64"
    fi
fi
